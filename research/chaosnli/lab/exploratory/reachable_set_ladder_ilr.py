"""Module 1: Gauge-Invariant ILR E018 Ladder & Fold-Coherent Relational Evaluation.
...
- Tier 4 (Nonlinear ILR MLP): x_i' = MLP(x_i) trained in ILR space with early stopping.
...
def run_reachable_set_ladder_ilr() -> dict:
    ...
    X_human_ilr = probs_to_ilr(P_human)
    ...
    for mname in target_models:
        ...
        # Raw posterior support Q_support_raw
        Q_support_raw = float(np.mean(np.sum(W_human * S_posterior, axis=1) / 10.0))
        ...
        # Fold-coherent graphs for test focal row scoring
        ...
        for fold, (train_idx, test_idx) in enumerate(splits):
            ...
            P_te, X_te = P_human[test_idx], X_raw_ilr[test_idx]
            ...
            # Full dataset transformation for fold coherence
            ...
            # --- Tier 4: Nonlinear ILR MLP (MLP in ILR space) ---
        ...
        def eval_oof(Q_oof, W_focal):
            ...
            # Fold-coherent posterior support recovery Q_support
    ...
    # Save per-item OOF predictions for E019
    ...
    return summary, oof_data_by_model

if __name__ == "__main__":
    out_dir = Path("results/exploratory")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    summary, oof_data = run_reachable_set_ladder_ilr()
    out_file = out_dir / "reachable_set_ladder_summary.json"
    with open(out_file, "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)
    print(f"Gauge-Invariant ILR E018 Reachable-Set Ladder summary written to {out_file}")
