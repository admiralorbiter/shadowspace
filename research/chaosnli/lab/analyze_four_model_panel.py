"""E004 Stage 2 Complete 4-Model Panel Synthesis & Coalition Census.

Evaluates Gemma 3 12B, Gemma 4 12B, Qwen 2.5 14B, and Qwen3 14B on pilot_600:
  1. 15-Coalition Census (Single models, 2-model, 3-model, and 4-model grand coalition).
  2. Linear and Logarithmic Opinion Pooling for all multi-model coalitions.
  3. Paired 30-stratum bootstrap CIs for all coalitions against Qwen 2.5 alone.
  4. 2x2 Generational x Family Matrix Synthesis.
  5. Four-Model Edge Atlas & Pareto Coordinates (NLL vs R_norm).
"""
...
def main():
    ...
    # All 15 Coalition Combinations (2^4 - 1)
    coalition_specs = [
        ["G3"], ["G4"], ["Q2.5"], ["Q3"],
        ["G3", "G4"], ["G3", "Q2.5"], ["G3", "Q3"], ["G4", "Q2.5"], ["G4", "Q3"], ["Q2.5", "Q3"],
        ["G3", "G4", "Q2.5"], ["G3", "G4", "Q3"], ["G3", "Q2.5", "Q3"], ["G4", "Q2.5", "Q3"],
        ["G3", "G4", "Q2.5", "Q3"]
    ]
    ...
    q25_q_rows_cal = q25_res["q_rows_cal"]
    q25_null_cal = q25_res["null_by_item_cal"]
    ...
    for spec in coalition_specs:
        ...
        for p_type in pool_types:
            ...
            # Paired 30-stratum bootstrap CI against Qwen 2.5 alone
            boot_diff_cal = []
            for _ in range(1000):
                boot_idx_list = []
                ...
                null_q25_b = float(np.mean(q25_null_cal[idx_boot]))
                r_q25_b = float((np.mean(q25_q_rows_cal[idx_boot]) - null_q25_b) / (q_hh_k10 - null_q25_b) * 100.0)
                ...
                boot_diff_cal.append(r_c_b - r_q25_b)

            ci_diff_vs_q25 = [float(np.percentile(boot_diff_cal, 2.5)), float(np.percentile(boot_diff_cal, 97.5))]
            c_res["delta_r_vs_qwen25_calibrated_pct"] = c_res["r_norm_pct_calibrated"] - q25_res["r_norm_pct_calibrated"]
            c_res["delta_r_vs_qwen25_calibrated_95ci"] = ci_diff_vs_q25

            coalition_results[key] = c_res
            ser_c = {k_sub: v_sub for k_sub, v_sub in c_res.items() if not isinstance(v_sub, np.ndarray)}
            serializable_coalitions[key] = ser_c

    # 2x2 Generational x Family Matrix Summary
    family_matrix = {
        "gemma_family": {
            "gemma3_12b": {"raw_nll": g3_res["nll_raw_nats"], "cal_nll": g3_res["nll_calibrated_nats"], "raw_r": g3_res["r_norm_pct_raw"], "cal_r": g3_res["r_norm_pct_calibrated"], "within_cal_gain": g3_res["within_model_calibration_gain_pct"]},
            "gemma4_12b": {"raw_nll": g4_res["nll_raw_nats"], "cal_nll": g4_res["nll_calibrated_nats"], "raw_r": g4_res["r_norm_pct_raw"], "cal_r": g4_res["r_norm_pct_calibrated"], "within_cal_gain": g4_res["within_model_calibration_gain_pct"]},
            "generational_delta_r_raw": g4_res["r_norm_pct_raw"] - g3_res["r_norm_pct_raw"],
            "generational_delta_r_cal": g4_res["r_norm_pct_calibrated"] - g3_res["r_norm_pct_calibrated"]
        },
        "qwen_family": {
            "qwen2.5_14b": {"raw_nll": q25_res["nll_raw_nats"], "cal_nll": q25_res["nll_calibrated_nats"], "raw_r": q25_res["r_norm_pct_raw"], "cal_r": q25_res["r_norm_pct_calibrated"], "within_cal_gain": q25_res["within_model_calibration_gain_pct"]},
            "qwen3_14b":   {"raw_nll": q3_res["nll_raw_nats"], "cal_nll": q3_res["nll_calibrated_nats"], "raw_r": q3_res["r_norm_pct_raw"], "cal_r": q3_res["r_norm_pct_calibrated"], "within_cal_gain": q3_res["within_model_calibration_gain_pct"]},
            "generational_delta_r_raw": q3_res["r_norm_pct_raw"] - q25_res["r_norm_pct_raw"],
            "generational_delta_r_cal": q3_res["r_norm_pct_calibrated"] - q25_res["r_norm_pct_calibrated"]
        }
    }
    ...
    summary = {
        "title": "E004 Stage 2 Complete 4-Model Panel Synthesis & Coalition Census",
        "generational_family_2x2_matrix": family_matrix,
        "coalition_census": serializable_coalitions,
        "pareto_pointwise_relational_coordinates": pareto_points
    }

    out_path = PROJECT_ROOT / "research" / "chaosnli" / "results" / "E004_four_model_panel_summary.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    print("\n============================================================")
    print("  STAGE 2 COMPLETE 4-MODEL PANEL COALITION CENSUS")
    print("============================================================")
    print("  2x2 Generational x Family Matrix:")
    print(f"    Gemma 3 12B  : Raw NLL={g3_res['nll_raw_nats']:.4f} | Cal NLL={g3_res['nll_calibrated_nats']:.4f} | Raw R={g3_res['r_norm_pct_raw']:.2f}% | Cal R={g3_res['r_norm_pct_calibrated']:.2f}% | Cal Gain={g3_res['within_model_calibration_gain_pct']:+.2f}%")
    print(f"    Gemma 4 12B  : Raw NLL={g4_res['nll_raw_nats']:.4f} | Cal NLL={g4_res['nll_calibrated_nats']:.4f} | Raw R={g4_res['r_norm_pct_raw']:.2f}% | Cal R={g4_res['r_norm_pct_calibrated']:.2f}% | Cal Gain={g4_res['within_model_calibration_gain_pct']:+.2f}%")
    print(f"    Qwen 2.5 14B : Raw NLL={q25_res['nll_raw_nats']:.4f} | Cal NLL={q25_res['nll_calibrated_nats']:.4f} | Raw R={q25_res['r_norm_pct_raw']:.2f}% | Cal R={q25_res['r_norm_pct_calibrated']:.2f}% | Cal Gain={q25_res['within_model_calibration_gain_pct']:+.2f}%")
    print(f"    Qwen3 14B    : Raw NLL={q3_res['nll_raw_nats']:.4f} | Cal NLL={q3_res['nll_calibrated_nats']:.4f} | Raw R={q3_res['r_norm_pct_raw']:.2f}% | Cal R={q3_res['r_norm_pct_calibrated']:.2f}% | Cal Gain={q3_res['within_model_calibration_gain_pct']:+.2f}%")
    print("------------------------------------------------------------")
    print("  Top Multi-Model Coalition Highlights (Calibrated R):")
    for k in ["G3+G4_linear", "G4+Q2.5_linear", "G4+Q3_linear", "Q2.5+Q3_linear", "G3+G4+Q2.5+Q3_linear", "G3+G4+Q2.5+Q3_logarithmic"]:
        if k in coalition_results:
            c = coalition_results[k]
            print(f"    {k:28s}: NLL={c['nll_calibrated_nats']:.4f} nats | R_cal={c['r_norm_pct_calibrated']:.2f}% | vs Q2.5: {c['delta_r_vs_qwen25_calibrated_pct']:+.2f}% (95% CI: [{c['delta_r_vs_qwen25_calibrated_95ci'][0]:+.2f}%, {c['delta_r_vs_qwen25_calibrated_95ci'][1]:+.2f}%])")
    print("============================================================")
    print(f"Exported complete 4-model panel synthesis to {out_path}")
